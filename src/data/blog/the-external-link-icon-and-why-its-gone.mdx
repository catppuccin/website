...
The CSS based approach is less code and less overhead, but we decided against it
as a number of exceptions (images/badges as links) had to be made which we, at
the time, thought could be better handled via a component with props.
...
We took the opportunity to add the mapping between the `<a>` tag and the new
`<Link>` component:
...
icon was removed (therefore also removing the need for the component entirely.)
